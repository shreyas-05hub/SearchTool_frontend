...
export function PreviewModal({doc, onClose}){
  ...
  return (
    <div style={{
      ...
      <div style={{background:'#fff', width:'80%', maxHeight:'80%', overflow:'auto', padding:20, borderRadius:8}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
          <h3 style={{margin:0}}>{doc.title}</h3>
          <button onClick={onClose} style={{padding:'6px 8px'}}>Close</button>
        </div>
        <pre style={{whiteSpace:'pre-wrap', marginTop:12, color:'#222'}}>{doc.content || doc.snippet}</pre>
      </div>
    </div>
  );
}
